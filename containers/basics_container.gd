...
@onready var csharp_checkbox: CheckBox = %CSharpCheckbox
...
func _setup_godot_versions() -> void:
	...
	if exit_code != 0:
		ErrorHandler.show_fatal_error("GDVM is not installed, not found in PATH or failed")
	var lines := output[0].replace("\r\n", "\n").split("\n")
	for line in lines:
		if not line.begins_with("-"):
			continue
		godot_version_option.add_item(line.replace("- ", ""))
